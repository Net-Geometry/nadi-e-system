...
export const useInsertBillingData = () => {
  ...
  const insertBillingData = async (utilityData: any, selectedFile: File | null, utilityTypes: any[], siteId: string) => {
    ...
    try {
      ...
      if (selectedFile) {
        const fileName = `${utilityData.year}_${utilityData.month}_${utilityTypes.find(type => type.id === parseInt(utilityData.type_id))?.name}.pdf`;
  ...
  const updateBillingData = async (utilityId: number, utilityData: any, selectedFile: File | null, utilityTypes: any[], siteId: string) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        const existingFilePath = existingAttachment.file_path.replace(`${SUPABASE_URL}/storage/v1/object/public/${BUCKET_NAME_UTILITIES}/`, '');
